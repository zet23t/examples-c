...
"- The Dijkstra algorithm can be improved by directing the search\n"
"- Instead of dequeuing the node with the lowest costs, ...\n"
"- ... we dequeue the node with the best prospect of reaching the end\n"
"- This is done by adding a heuristic to the costs\n"
"   - The heuristic function tries to estimate the remaining costs\n"
"   - The function should not overestimate the remaining costs\n"
"   - It is often the euclidean distance to the end point\n"
"   - In a rectangular grid, the Manhattan distance is a good choice\n"
"- The Dijkstra search can be emulated by A* by letting the heuristic\n"
"  function return a constant value\n"
"- A* is typically very efficient\n"
"   - If the target however can't be reached, it makes a full search\n"
"   - In this worst case, it is as slow as the broad search\n"
"- A* can be used outside of pathfinding as well"
;
...
void page_5_onInit(TutorialPage *page) {}
void page_5_onEnter(TutorialPage *page) 
{
    subStep = 0;
}
void page_5_onLeave(TutorialPage *page) {}
void page_5_onUpdate(TutorialPage *page, float dt) 
{
    subStepTime += dt;
}
...
void page_5_onDraw(TutorialPage *page, float dt)
{
...
bool page_5_onNextSubstep(TutorialPage *page)
{
    subStepTime = 0.0f;
